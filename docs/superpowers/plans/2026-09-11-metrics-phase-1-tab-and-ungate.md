...
- **Read `DESIGN.md` before any visual decision**, starting with its "Reference boundary and migration stance" banner (corrected 2026-09-11: an earlier draft of this plan cited a "clone agent-orchestrator verbatim" banner that does not exist in this file — that language belongs to a different CLAUDE.md and does not apply here). The real banner says existing rounded cards, per-block borders, shadows, and one-off styling are migration debt, not precedent, and warns against copying a legacy inconsistency into new work. Task 3's verbatim extraction of existing components is accepted as a one-time relocation exemption from that rule (it is a move, not new styling), but any genuinely new visual surface in a later phase must be held to the actual banner's standard, not this citation. Build the new tab from the existing `@aoagents/product-ui` primitives and `components/ui/*` shadcn components; the tab icon and section layout must match the four tabs already there. Do not introduce new visual patterns without explicit approval.
